...
def export_devices_excel():
    ...
    try:
        ...
        for stack_device in stack_devices:
            ...
            # 总接口数统计
            total_interfaces = len([i for i in interfaces if i.get('type') != 'management'])
            stack_interface_count = len(stack_links)
            business_interface_count = total_interfaces - stack_interface_count
            ...
            stack_export_data.append({
                ...
                '管理接口': management_interface or '未配置',
                ...
                '业务接口数': business_interface_count,
                '堆叠互联接口数': stack_interface_count,
                '聚合口配置': agg_summary,
    ...
    def setup_worksheet_style(ws, sheet_type):
            ...
            for row_idx, row in enumerate(ws.iter_rows(min_row=2), start=2):
                # 设置行高以适应多行文本
                ws.row_dimensions[row_idx].height = 60  # 增加行高
            ...
            if sheet_type == '独立设备配置':
                ...
                column_widths = {
                    ...
                    '成员数量': 10, '成员设备': 50, '堆叠互联端口': 30, '管理接口': 15,
                    '带外管理IP': 16, '带内管理接口': 15, '带内管理IP': 16,
                    '业务接口': 15, '业务IPv4': 18, '业务IPv6': 25,
                    '总接口数': 10, '业务接口数': 12, '堆叠互联接口数': 15,
                    '聚合口配置': 35, '堆叠ID': 12
                }
            ...
            for col_idx, column in enumerate(ws.columns, start=1):
                ...
                width = column_widths.get(header, 15)  # 默认宽度15
